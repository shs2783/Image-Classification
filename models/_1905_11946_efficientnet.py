...
class SEBlock(nn.Module):
    def __init__(self, channels, reduction_channels) -> None:
        super().__init__()
...
class MBConvBlock(nn.Module):  # InvertedResidual + SE
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, expansion=6, reduction_ratio=4) -> None:
        super().__init__()

        self.residual = (in_channels == out_channels)
        hidden_channels = expansion * in_channels
        reduction_channels = int(in_channels / reduction_ratio)
        ...
        self.bottleneck_layer = nn.Sequential(
            ...
            SEBlock(hidden_channels, reduction_channels),
...
class EfficientNet(nn.Module):
    def __init__(self, config=efficientnet_architecture, depth_multiplier=1.2, width_multiplier=1.1, reduction_ratio=4, in_channels=3, num_classes=1000) -> None:
        super().__init__()
    ...
    def _create_layers(self, architecture, reduction_ratio=4, depth_multiplier=1.2, width_multiplier=1.1):
        layers = []
...
if __name__ == '__main__':
    ...
    # TODO 2: calculate detph, width, resolution multiplier
    ...
    model = EfficientNet(depth_multiplier=depth_multiplier, width_multiplier=width_multiplier, reduction_ratio=4)
